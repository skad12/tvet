...
export const useUserStore = create(
  persist(
    (set, get) => ({
      ...
      // Fetch tickets (optionally scoped by user id or email) and cache them
      fetchTickets: async (opts = {}) => {
        const {
          userId = null,
          userEmail = null,
        ...
        try {
          let data;
          // Prioritize email-based fetching for customers who logged in via OTP
          if (userEmail) {
            // Filter tickets by email - use POST method with email in body
            const res = await api.post(`/filter-ticket/by-user-email/`, {
              email: userEmail,
            });
            data = res?.data;
          } else if (userId) {
      ...
      fetchTicketsIfNeeded: async (opts = {}) => {
        ...
        const isStale = Date.now() - last > ttlMs;
        if (force || !hasAny || isStale) {
          return await get().fetchTickets({ userId, userEmail, start, stop });
        }
        return get().tickets || [];
      },
