...
export default function ChatList({
  ...
  function normalizeTicket(t) {
    ...
    const fallbackStatus =
      statusBool ?? ticketStatusStr ?? statusStr ?? t?.state ?? null;
  ...
  useEffect(() => {
    ...
    async function load() {
      ...
      const endpointRelative = effectiveUserId
        ? `/filter-ticket/by-user-id/${effectiveUserId}/`
        : `/tickets/`;
...
//   // Fetch or normalize prop tickets
//   useEffect(() => {
//     // When tickets provided via prop and we are not forced to fetch from server:
//     if (ticketsProp && !forceFetch) {
//       const arr = Array.isArray(ticketsProp) ? ticketsProp : [ticketsProp];
//       const normalized = arr.map(normalizeTicket);
//       const shouldFilter = Boolean(effectiveUserId || effectiveUserEmail);
...
//       const endpointRelative = `/filter-ticket/by-user-id/${String(
//         effectiveUserId
//       )}/`;
...
//         // server endpoint should already be filtered by user id, but double-check
//         const shouldFilter = Boolean(effectiveUserId || effectiveUserEmail);
//         const filteredByUser = shouldFilter
//           ? normalized.filter((n) =>
//               ticketBelongsToUser(n, effectiveUserId, effectiveUserEmail)
...
//     () => [
//       columnHelper.accessor("displaySubject", {
//         header: "Subject",
//         cell: (info) => {
...
//               </div>
//               <div className="text-xs text-slate-500 mt-1 truncate">
//                 {row.email || row.name || "From Widget"}
//               </div>
//             </div>
//           );
//         },
...
//         cell: (info) => {
//           const row = info.row.original;
//           const statusKey = (row.status ?? "active").toLowerCase();
//           const pillClass =
//             statusKey === "resolved"
//               ? "bg-emerald-50 text-emerald-700 border border-emerald-100"
//               : statusKey === "waiting" || statusKey === "pending"
//               ? "bg-amber-50 text-amber-700 border border-amber-100"
//               : "bg-sky-50 text-sky-700 border border-sky-100";
//           const label =
//             row.preview ||
...
//         (row) => formatMaybeDate(row.created_at, row.created_at_display),
//         {
//           id: "created_at",
//           header: "Created",
//           cell: (info) => (
//             <div className="text-xs text-slate-400">{info.getValue()}</div>
//           ),
//         }
//       ),
...
//           const assignedUser =
//             directory.find(
//               (p) => p?.id && String(p.id) === String(assignedToId)
//             ) ?? null;
//           const label =
//             assignedUser?.name && assignedUser.name !== "null"
//               ? assignedUser.name
//               : assignedUser?.username ??
//                 assignedUser?.email ??
//                 info.row.original.raw?.assigned_to_name ??
//                 "Unassigned";
//           return <div className="text-xs text-slate-500">{label}</div>;
//         },
//       }),
//     ],
//     [columnHelper, directory]
//   );
...
//           ) : (
//             <ul className="space-y-3">
//               {/* Render each row using table.getRowModel().rows so sorting works */}
...
//                   directory.find(
//                     (person) =>
//                       person?.id && String(person.id) === String(t.assignedTo)
//                   ) ?? null;
//                 const assignedLabel =
//                   assignedUser?.name && assignedUser.name !== "null"
//                     ? assignedUser.name
