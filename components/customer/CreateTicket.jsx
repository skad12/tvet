...
//       (() => {
//         try {
//           return localStorage.getItem("tvet_user_email") || "";
//         } catch (err) {
//           return "";
//         }
//       })();
//     setReporterEmail(resolved);
//   }, [isOpen, authEmail, defaultReporterEmail]);
...
export default function CreateTicket({
  ...
  const [categoryId, setCategoryId] = useState("");
  const [subject, setSubject] = useState(""); // optional now
  const [priority, setPriority] = useState("Low"); // Low | Medium | High | Urgent
  const [description, setDescription] = useState(""); // will be sent as `message`
  const [reporterEmail, setReporterEmail] = useState("");
  ...
  const submit = async (ev) => {
    ...
    setLoading(true);
    try {
      // choose the category id from loaded categories (fall back to provided value)
      const selected = categories.find(
        (c) =>
          String(c.id ?? c._id ?? c.title ?? c.name ?? c.label ?? "") ===
          String(categoryId ?? "")
      ) ?? { id: categoryId };
      ...
      // Include subject if the user provided one (optional)
      ...
      // clear fields (preserve category default)
      setSubject("");
      setPriority("Low");
      setDescription("");

      // optionally redirect or just close (short delay for UX)
      setTimeout(() => {
        if (redirectAfter) {
          router.push(redirectAfter);
        } else {
