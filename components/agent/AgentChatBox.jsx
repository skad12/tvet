...
export default function ChatBox({
  ...
  async function retryMessage(msg) {
    ...
    try {
      ...
      setMessages((prev) => prev.map((m) => (m.id === msg.id ? { ...m, status: "sent", role: CURRENT_ROLE } : m)));
      digestRef.current = "";
      fetchChats();
      requestAnimationFrame(scrollToBottom);
    } catch (err) {
    ...
    >
      ...
      {!selected ? (
        ...
        <>
                ...
                <button
                  onClick={async (e) => {
                    ...
                    try {
                      ...
                      try {
                        if (typeof onEscalated === "function") onEscalated(selected.id);
                      } catch (e) {}
                    } catch (e) {
                      setEscalateError(e?.message ?? "Failed to escalate ticket");
                      setEscalated(false);
                  ...
                  aria-label="Escalate ticket"
                >
                  {escalating ? "Escalating…" : escalated ? "Escalated" : "Escalate"}
                </button>
                      ...
                      >
                        ...
                        {m.status === "failed" && (
                          <>
                            ...
                            <button onClick={() => retryMessage(m)} className="ml-2 text-xs underline hover:opacity-80">
                              Retry
                            </button>
                          </>
                        )}
          ...
          {serverResponseSnippet && (
            <div className="mt-2 text-xs">
              <button onClick={() => setShowServerResponse((s) => !s)} className="underline text-blue-600">
                {showServerResponse ? "Hide server response" : "Show server response (debug)"}
              </button>
              {showServerResponse && (
                <pre className="mt-2 p-2 bg-slate-100 rounded overflow-auto text-xs max-h-40">{serverResponseSnippet}</pre>
              )}
            </div>
          )}
        </>
