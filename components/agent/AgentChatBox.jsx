...
export default function AgentChatBox({ selected, userEmail: propUserEmail }) {
  const { token, user } = useAuth();
  const userEmail = propUserEmail ?? user?.email ?? user?.username ?? "me";
        ...
        <>
          ...
          >
            <motion.div
              initial={{ opacity: 0, y: 6 }}
              animate={{ opacity: 1, y: 0 }}
              className="bg-blue-600 text-white rounded-lg p-3 text-sm shadow-sm max-w-[75%]"
            >
              Support — Welcome to HelpDesk! Your ticket is being routed, an
              agent will join shortly.
            </motion.div>
            ...
            <AnimatePresence initial={false}>
              ...
              {messages.map((m, idx) => {
                // treat 'agent' messages (or the CURRENT_ROLE) as agent messages
                ...
                const isSameSender =
                  prevMessage && (isAgent ? prevIsAgent : !prevIsAgent);
                ...
                return (
                  <motion.div
                    ...
                    animate="visible"
                    exit="exit"
                    // align right for agent, left for customer
                    className={`flex ${
                      isAgent ? "justify-end" : "justify-start"
                    } ${isSameSender ? "mt-0.5" : "mt-2"}`}
                  >
                    <div
                      className={`p-3 rounded-lg max-w-[75%] shadow-sm flex flex-col ${
                        isAgent
                          ? "bg-blue-600 text-white rounded-tr-none items-end"
                          : "bg-gray-200 text-gray-800 rounded-tl-none items-start"
                      }`}
                    >
                      <div className="text-sm whitespace-pre-wrap break-words">
                        {m.text}
                      ...
                      <div
                        className={`text-xs mt-1.5 flex items-center gap-2 ${
                          isAgent ? "text-blue-100" : "text-gray-600"
                        }`}
                      >
                        <span>{format(new Date(m.at), "h:mm a")}</span>
                        {m.status === "pending" && (
