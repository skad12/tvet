...
export default function TicketsTable({ onOpenChat, selectedId }) {
  ...
  function normalizeTicketForTable(t) {
    ...
    if (t?.escalated === true) {
      ...
      status = t.status ? "resolved" : (t?.progress ? String(t.progress).toLowerCase() : "pending");
    } else if (t?.progress) {
      ...
      try {
        ...
        try {
          const statusPromises = normalized.map(async (ticket) => {
            ...
            try {
              const statusRes = await api.get(`/get-ticket-status/${ticket.id}/`, {
                signal: ac.signal,
              });
              const statusData = statusRes?.data;
              if (statusData) {
                // Update status from API response
                const newStatus = statusData.status ?? statusData.progress ?? null;
                if (newStatus) {
                  return {
                    ...ticket,
                    status: String(newStatus).toLowerCase(),
                    raw: { ...ticket.raw, status: newStatus, progress: statusData.progress },
                  };
                }
              }
            } catch (err) {
              ...
              if (err?.name !== "AbortError") {
                console.warn(`Failed to fetch status for ticket ${ticket.id}:`, err);
              }
            }
            return ticket;
          });
        ...
        const counts = normalized.reduce((acc, ticket) => {
          const name = ticket.categoryName ?? ticket.categoryTitle ?? "From Widget";
          ...
          return (
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                {row.original.escalated && String(row.original.status).toLowerCase() !== "resolved" && (
                  <div className="mt-1">
          ...
          {categoryFilters.map((filter) => {
            ...
            return (
              ...
              >
                ...
                <span className="inline-flex items-center justify-center min-w-[28px] px-2 py-0.5 rounded-full bg-white/20 border border-white/30">
                  {filter.count}
                </span>
              </button>
            );
