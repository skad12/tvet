...
//   // --- NEW: state for remove action (minimal additions only) ---
...
//   // --- NEW: remove agent handler (minimal & safe) ---
//   async function handleRemoveAgent(agent) {
//     setRemoveError(null);
//     setRemoveSuccess(false);

//     if (!agent || !activeCategory) return;

//     const agentId = agent.id;
//     const categoryId = activeCategory.id ?? activeCategory._id;

//     // don't attempt to remove if there's no usable id (fallback assigned placeholder)
//     if (!agentId || String(agentId) === String(categoryId)) {
//       setRemoveError("This agent cannot be removed (no valid user id).");
//       return;
//     }

//     const ok = window.confirm(
//       `Remove ${agent.username || "this agent"} from category "${
//         activeCategory.title || activeCategory.name || ""
//       }"?`
//     );
//     if (!ok) return;
...
//       await api.post("/remove-user/from-category/", {
//         category_id: categoryId,
...
//                           {/* Status + REMOVE BUTTON (added only) */}
//                           <div className="flex items-center gap-2">
//                             <span
//                               className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[11px] font-medium border ${badgeClass}`}
//                             >
...
//                               className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-transparent hover:bg-red-50 disabled:opacity-50"
//                               title="Remove agent from category"
//                               aria-label={`Remove ${agent.username || "agent"}`}
//                             >
...
//                                 </svg>
//                               ) : (
//                                 // Trash icon (kept simple and inline)
//                                 <svg
...
//       {/* NEW: remove feedback (minimal placement only) */}
//       {removeSuccess && (
//         <div className="mt-4 p-3 rounded-md bg-emerald-50 text-emerald-700 text-sm border border-emerald-200">
//           Agent removed from category successfully
//         </div>
//       )}
...
//     // 1) First try existing link-based approach (agents objects from allAgents)
//     const fromLink = agentsForCategory(activeCategory, allAgents || []);
//     if (fromLink.length > 0) return fromLink;

//     // 2) Next, if the API returns `assigned_agents` (array of names), display them.
//     //    Try to resolve names to real agent objects from allAgents (case-insensitive match on username).
//     const assignedList = Array.isArray(activeCategory.assigned_agents)
//       ? activeCategory.assigned_agents
//       : null;

//     if (assignedList && assignedList.length > 0) {
//       const mapped = [];
//       for (let i = 0; i < assignedList.length; i++) {
//         const raw = assignedList[i];
//         const name = normalizeAgentName(raw);
//         if (!name) continue;

//         // try to find matching agent object in allAgents by username (case-insensitive)
//         const matched =
//           (allAgents || []).find(
//             (a) =>
//               (a.username || "")
...
//           mapped.push(matched);
//         } else {
//           // synthetic entry: has no real id so remove button will not be shown for it
//           mapped.push({
//             id: null,
//             username: name,
//             email: "",
...
//     // 3) Fallback: only show a single assigned agent if the assigned_agent field
//     // contains a valid name/value (not "null", not empty, etc).
//     const assignedName = normalizeAgentName(activeCategory.assigned_agent);
//     if (assignedName) {
//       return [
//         {
//           id: null, // synthetic (no real id)
//           username: assignedName,
//           email: "",
//           phone: "—",
//           status: "available",
...
//                             {/* show remove button ONLY when agent has a real id (prevents attempts to remove synthetic name-only entries) */}
//                             {agent.id ? (
//                               <button
//                                 onClick={() => handleRemoveAgent(agent)}
//                                 disabled={removingAgentId === agent.id}
...
export default function AgentCategoriesList({
  ...
  // builds the activeAgents array for the selected category (preserves previous behavior)
  const activeAgents = useMemo(() => {
    if (!activeCategory) return [];
  ...
  // ----------------- NEW: helper to compute count for a category -----------------
  // Uses the same logic as the number next to the Assign Agent button:
  // 1) linked agents from allAgents
  // 2) length of assigned_agents after filtering invalid names
  // 3) fallback to assigned_agent single
  function countForCategory(cat) {
    if (!cat) return 0;

    const linked = agentsForCategory(cat, allAgents || []).length;
    if (linked > 0) return linked;

    const assignedList = Array.isArray(cat.assigned_agents)
      ? cat.assigned_agents
      : null;
    if (assignedList && assignedList.length > 0) {
      const valid = assignedList.filter((n) => normalizeAgentName(n) !== null);
      if (valid.length > 0) return valid.length;
    }

    if (isAssignedAgentValid(cat.assigned_agent)) return 1;
          ...
          <div className="mb-4 overflow-x-auto pb-1">
            <div className="inline-flex items-center gap-2">
              {(categories || []).map((cat) => {
                ...
                // <-- use the same count logic as the assign button shows -->
                          ...
                          <div className="flex items-center gap-2">
                            ...
                            {agent.id ? (
                              <button
                                onClick={() => handleRemoveAgent(agent)}
                                disabled={removingAgentId === agent.id}
                                className="inline-flex items-center justify-center w-8 h-8 rounded-full border border-transparent hover:bg-red-50 hover:scale-105 transition-transform duration-150 disabled:opacity-50"
                                title="Remove agent from category"
                                aria-label={`Remove ${
                                  agent.username || "agent"
                                }`}
