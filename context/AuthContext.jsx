...
//         normalizeRoleValue(returnedUser?.account_type) ??
//         null;

//       if (normalizedRole) {
//         returnedUser.account_type =
//           returnedUser.account_type ?? normalizedRole;
//         returnedUser.role = returnedUser.role ?? normalizedRole;
//       }
...
//           if (normalizedRole) localStorage.setItem("account_type", normalizedRole);
//           if (appUserId) localStorage.setItem("user_id", String(appUserId));
//           const primaryEmail =
//             returnedUser?.email ??
//             returnedUser?.username ??
//             (Array.isArray(returnedUser?.emails)
//               ? returnedUser.emails[0]
//               : null);
//           if (primaryEmail) {
//             localStorage.setItem("tvet_user_email", String(primaryEmail));
//           }
//         }
//       } catch (err) {
...
// inside AuthContext - signIn implementation
// async function signIn({ username, password }) {
//   setLoading(true);
//   try {
//     const payload = { username, password };
//     const tryEndpoints = ["/sign-in/", "/sign-in"];
//     let lastErr = null;
...
//               }
//             ).then((r) =>
//               r.json().then((d) => ({ status: r.status, data: d }))
//             );

//         // normalize axios-style response vs fetch wrapper
//         const status = res?.status ?? (res?.status === 0 ? 200 : undefined);
...
//         // success path — expected shape: { token, id, account_type }
//         const token = data?.token || data?.access || data?.access_token || null;
//         const returnedUser =
//           data?.user ?? { id: data?.id, account_type: data?.account_type } ??
//           data;
...
//         setUser(returnedUser);
//         return { ok: true, token, user: returnedUser, raw: data };
//       } catch (err) {
//         lastErr = err;
//         // try next endpoint
//       }
...
//     // all attempts failed — return normalized error
...
//       message = `Sign-in failed (status ${lastErr.status})`;
//     } else if (lastErr?.message) {
//       message = lastErr.message;
//     }
//     console.error("signIn error:", lastErr);
//     return { ok: false, error: message, debug: lastErr };
//   } finally {
//     setLoading(false);
//   }
// }
